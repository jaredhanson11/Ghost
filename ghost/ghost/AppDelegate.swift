...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didReceiveRemoteNotification userInfo: [NSObject : AnyObject]) {
        // notify client to call mainPage API to receive new message
        let a = window!.rootViewController as! UINavigationController
        let controllers = a.viewControllers
        for i in 0...controllers.count-1 {
            print(controllers[i].title)
        }
        let push = userInfo as! [String:AnyObject]
        print(push)
//        if (mainViewController.isViewLoaded()) {
//            mainViewController.newMessage = true
//        } else {
//            print("main view not loaded")
//        }
    }
}